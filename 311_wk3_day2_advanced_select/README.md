...
We're starting to get pretty advanced with our SQL queries. Keep researching other advanced SELECT statements and get ready to foray into [INSERTs and UPDATEs and DELETEs, oh CRUD](https://www.youtube.com/watch?v=-HrfbV16-FQ)!.
	  ...
	  SELECT SUM(user_id) FROM db2.usersAddress;
    SELECT SUM(user_id) FROM db2.usersContact;
    ...
    SELECT MIN(first_name), county, COUNT(u.id) FROM users AS u 
    JOIN usersAddress AS ua WHERE u.id = ua.user_id GROUP BY county
    HAVING COUNT(ua.user_id) > 10;